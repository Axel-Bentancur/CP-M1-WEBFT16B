...
var isAncestor = function (genealogyTree, ancestor, descendant) {
  ...
  for (i = 0; i < genealogyTree[ancestor].length; i++) {
...
LinkedList.prototype.switchPos = function (pos1, pos2) {
  ...
  if (pos1 > this.size() || pos2 > this.size() || pos1 < 0 || pos2 < 0) {
    ...
    let auxTwo = currentTwo.value;
    current.value = auxTwo;
    currentTwo.value = aux;
    return true;
  }
};
...
var mergeLinkedLists = function (linkedListOne, linkedListTwo) {
  ...
  while (currentOne != null && currentTwo != null) {
    list.add(currentOne.value);
    list.add(currentTwo.value);
...
/*if (target <= this.data && this.left !== null) {
